...
https://github.com/user-attachments/assets/38bfb0ee-e3a8-4e81-9f7f-82dda43626f9
...
https://github.com/user-attachments/assets/9f351846-3916-40da-9f69-9aae9a01e5b7
...
Cylinder shape and all other shapes are classes inherited from a new `shapes/shape.h` class that has all the common attributes (suh as tesselation parameters). 

Shaders: optimized by passing the `glm::inverse(glm::transpose(glm::mat3(m_model)))` as a uniform applied to normals only once so that it isn't recomputed for every vertex.

Camera data: A camera class is used and it makes it easy to move around by just translating or rotating the look, up and position and get the new view matrix, in `translateCamera` and `void rotateCamera`.

Projection matrix and view matrix are rebuilt when needed, by a call to dedicted functions, `updateProjMat` and `void updateViewMat`.

Camera Movement and Translation on Key Press are done in `timerEvent`.
Rotation on Mouse Movement in `mouseMoveEvent`.

Phong illumination, Point Lights w/ Attenuation, Spot Lights w/ Penumbra, Support for multiple lights. Nothing special, all in the fragment shader `defaul.frag`.
Tessellation (responds to changes in parameters) done by a call to `rebuildGeometry`.
